...
contract AuctionHouse is ReentrancyGuard, Pausable, Ownable {
    ...
    mapping(address => bool) public supportedTokens; // 支持的ERC20代币
     ...
     * @param _priceOracle 价格预言机地址
     */
    function initialize(address _feeRecipient, address _priceOracle) external {
        require(feeRecipient == address(0), "Already initialized");
        require(_feeRecipient != address(0), "Invalid fee recipient");
        require(_priceOracle != address(0), "Invalid price oracle");
        
        // 设置owner为调用者（通常是工厂合约的owner）
